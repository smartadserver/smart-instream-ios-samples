...
class ViewController: UIViewController, SVSAdManagerDelegate, PlayerViewControllerDelegate {
    ...
    private var shouldDisplayEndBumper:Bool = false
    ...
    func instantiateAdPlayerConfiguration() -> SVSAdPlayerConfiguration {
        ...
        // Disable autoplay to be able to display bumpers
        configuration.publisherOptions.enableAdBreakAutoplay = false
        // Force all ads to be not skippable
    ...
    func adManager(_ adManager: SVSAdManager, isReadyToStart adBreakType: SVSAdBreakType) {
        // Called when an AdBreak is ready to start (only if enableAdBreakAutoplay option is disabled).
        // This is where you should display your bumper "begin"
        
        // Here we pause the player
        playerViewController.pause()
        
        // We display the begin bumper
        playerViewController?.beginBumperView.isHidden = false
        
        // Here we hide the player controls to make sure they are not displayed over the ad.
        playerViewController?.showControls(false)
        
        // Display bumper for 3 seconds then start ad break
        let delayTime = DispatchTime.now() + .seconds(Constants.bumperDuration)
    ...
    func adManager(_ adManager: SVSAdManager, didStart adBreakType: SVSAdBreakType) {
        ...
        // Make sure to hide the begin bumper
        playerViewController?.beginBumperView.isHidden = true
    ...
    func adManager(_ adManager: SVSAdManager, didFinish adBreakType: SVSAdBreakType, numberOfAds numberOfAdsPlayed: Int, duration: TimeInterval, error: Error?) {
        ...
        if adBreakType == SVSAdBreakType.postroll && numberOfAdsPlayed > 0 {
            // Display the postroll's end bumper.
            playerViewController?.endBumperView.isHidden = false
            
            // After bumper duration, hide the bumper.
            let delayTime = DispatchTime.now() + .seconds(Constants.bumperDuration)
            DispatchQueue.main.asyncAfter(deadline: delayTime) {
                self.adManager?.startAdBreak()
                
                // Hide the postroll's end bumper.
                self.playerViewController?.endBumperView.isHidden = true
                
                // Here we do not hide the players controls anymore.
                self.playerViewController?.showControls(true)
            }
        } else {
            // Here, decide weither you should display end bumper or not, depending on the number of ads played
            shouldDisplayEndBumper = numberOfAdsPlayed > 0;
        }
    }
    ...
    func adManagerDidRequest(toResumeContentPlayer adManager: SVSAdManager, after adBreakType: SVSAdBreakType) {
        ...
        // But if you want to display end bumper, this is were to do it.
        
        if shouldDisplayEndBumper {
            // Reset flag
            shouldDisplayEndBumper = false
            
            // Display the end bumper
            self.playerViewController?.endBumperView.isHidden = false
            
            // After bumper duration, hide the bumper and resume the content as asked.
            let delayTime = DispatchTime.now() + .seconds(Constants.bumperDuration)
            DispatchQueue.main.asyncAfter(deadline: delayTime) {
                self.playerViewController?.endBumperView.isHidden = true
