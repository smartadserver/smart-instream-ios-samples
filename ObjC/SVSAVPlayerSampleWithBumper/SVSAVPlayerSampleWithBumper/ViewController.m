...
// Flag to decide weither we should display the end bumper or not.
@property (nonatomic, assign) BOOL shouldDisplayEndBumper;
...
- (SVSAdPlayerConfiguration *)instantiateAdPlayerConfiguration {
    ...
    configuration.publisherOptions.enableAdBreakAutoplay = NO;
    // Force all ads to be not skippable
    configuration.publisherOptions.forceSkipDelay = YES;
    configuration.publisherOptions.skipDelay = -1;
...
- (void)adManager:(SVSAdManager *)adManager isReadyToStartAdBreak:(SVSAdBreakType)adBreakType {
    ...
    self.beginBumperImageView.hidden = NO;
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kBumperDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        // After bumper duration, actually start the ad break.
...
- (void)adManager:(SVSAdManager *)adManager didStartAdBreak:(SVSAdBreakType)adBreakType {
    ...
    // Also make sure to hide the begin bumper
    self.beginBumperImageView.hidden = YES;
}
...
- (void)adManager:(SVSAdManager *)adManager didFinishAdBreak:(SVSAdBreakType)adBreakType numberOfAds:(NSInteger)numberOfAdsPlayed duration:(NSTimeInterval)duration error:(NSError *)error {
    ...
    if (adBreakType == SVSAdBreakTypePostroll && numberOfAdsPlayed > 0) {
        // Display the postroll's end bumper
        self.endBumperImageView.hidden = NO;
        
        // After bumper duration, hide the bumper.
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kBumperDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.endBumperImageView.hidden = YES;
...
- (void)adManagerDidRequestToResumeContentPlayer:(SVSAdManager *)adManager afterAdBreak:(SVSAdBreakType)adBreakType {
    // Called when the SVSAdManager wants you to play the content player. You should obey!
